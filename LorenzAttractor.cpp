...
LorenzAttractor::LorenzAttractor() {
    ...
    for (unsigned i = 0; i < num_points; i++)
    {
        ...
        point[i] = { getRandomNumber(0.0f, 10.0f), getRandomNumber(0.0f, 10.0f), getRandomNumber(0.0f, 10.0f) };
...
void LorenzAttractor::update() {
    /// Calculate timestep
    timestep = clock.getElapsedTime().asSeconds() * 100;
    ...
    for (unsigned g = 0; g < num_points; g++)
    {
        ...
        */
        ...
        while (i != static_cast<int>(j[g]))
        {
            ...
            // Increment counters
            i++;
            if (i == static_cast<int>(trail[g].size())) // Set i to 0 once it gets to the end of "trail"
                i = 0;
